(function () {
  ...
  function refreshAuthUI() {
    ...
    if (sideStatus) {
      sideStatus.textContent = currentUser
        ? ((currentUser.email || currentUser.name) + (premium ? " · Premium" : ""))
        : "Not signed in";
    }
    if (authArea) {
      if (currentUser) {
        var display = currentUser.name || (currentUser.email || "").split("@")[0];
  ...
  if (formSignin) {
    formSignin.addEventListener("submit", function (e) {
      ...
      currentUser = { email: u.email, name: u.name, isPremium: !!u.isPremium, premiumUntil: u.premiumUntil || null, redeemedCodes: u.redeemedCodes || [] };
  ...
  if (formSignup) {
    formSignup.addEventListener("submit", function (e) {
      ...
      users[key] = { email: key, name: name.trim(), password: password, isPremium: false, premiumUntil: null, redeemedCodes: [] };
      saveUsers(users);
      currentUser = { email: key, name: name.trim(), isPremium: false, premiumUntil: null, redeemedCodes: [] };
      setSession(currentUser);
